...
def start_command(message, *args, **kwargs):
    answer = (
        ...
        'Mən sürətləndirilmiş mafiya oyun botuyam.\n'
        '@JasperAzerbaijan'
    )
...
def stats_command(message, *args, **kwargs):
    ...
    if 'total' in stats:
        ...
        answer = (
            ...
            f'Qələbə: {win}/{stats["total"]} ({100 * win // stats["total"]}%)'
        )
        roles = []
        ...
        for role in sorted(roles, key=lambda s: s['rate'], reverse=True):
            answer += (
                ...
                f'Qələbə - {role.get("win", 0)}/{role["total"]} ({role["rate"]}%)'
            )
    ...
    if 'gallows' in stats:
        ...
        answer = f'Asılanları təxmin etdim: {right}/{right + wrong} ({100 * right // (right + wrong)}%)'
        answer += f'\nQələbə: {win}/{total} ({100 * win // total}%)'
        paragraphs.append(answer)
...
def rating_command(message, *args, **kwargs):
    ...
    paragraphs = []
    if mafia_rating:
        paragraphs.append('Mafiya oyunçu reytinqi::\n' + get_rating_list(mafia_rating))
    if croco_rating:
        paragraphs.append('Yerli oyunçu reytinqi::\n' + get_rating_list(croco_rating))
...
def play_croco(message, game, *args, **kwargs):
    if game:
        bot.send_message(message.chat.id, 'Bu söhbətdəki oyun artıq davam edir.')
        return
...
def play_gallows(message, game, *args, **kwargs):
    if game:
        if game['game'] == 'gallows':
            bot.send_message(message.chat.id, 'Bu söhbətdəki oyun artıq davam edir.', reply_to_message_id=game['message_id'])
        else:
            bot.send_message(message.chat.id, 'Bu söhbətdəki oyun artıq davam edir.')
        return
    word = croco.get_word()[:-2]
    ...
    if player_game:
        ...
        if player_object.get('role') is None:
            ...
            if len(players_without_roles) > 0:
                ...
                bot.edit_message_text(
                    'Oyunçuların oyun üçün sırası belədir:\n\n' + format_roles(player_game),
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                )
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text='İndi bu qarışıqlıqda oyun oynamırsan.'
        )
...
def mafia_team(call):
    ...
    if player_game:
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=True,
            text='Siz növbəti komandada oynayırsınız:\n' +
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text='Mafiya heyəti ilə görüşə bilməzsiniz.'
        )
...
def check_don(call):
    ...
    if player_game and call.from_user.id not in player_game['played']:
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=True,
            text=f'Bəli, oyunçu nömrələndi {check_player + 1} - {role_titles["sheriff"]}'
                 if player_game['players'][check_player]['role'] == 'sheriff' else
                 f'Xeyr, oyunçu nömrələnmədi {check_player + 1} - {role_titles["sheriff"]}'
        )
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text='Donun kartını əldə edə bilməzsən.'
        )
...
def check_sheriff(call):
    ...
    if player_game and call.from_user.id not in player_game['played']:
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=True,
            text=f'Bəli, oyunçu nömrələndi {check_player + 1} - {role_titles["don"]}'
                 if player_game['players'][check_player]['role'] == 'don' else
                 f'Bəli, oyunçu nömrələndi {check_player + 1} - {role_titles["mafia"]}'
                 if player_game['players'][check_player]['role'] == 'mafia' else
                 f'Xeyr, oyunçu nömrələnmədi {check_player + 1} - {role_titles["mafia"]}'
        )
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text='Komisarın kartını əldə edə bilməzsiniz.'
        )
...
def append_order(call):
    ...
    if player_game:
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text=f'Oyunçu nömrəsi {call_player} sifarişə əlavə edildi.'
        )
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text='Don əmrini verə bilməzsiniz.'
        )
...
def vote(call):
    ...
    if player_game and call.from_user.id not in player_game['played']:
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text=f'Səs oyunçuya qarşıdır {vote_player + 1}.' if vote_player >= 0 else 'Səsli aktyor.'
        )
...
def end_order(call):
    ...
    if player_game:
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text='Sifariş qeydə alınıb və mafiya komandasına göndəriləcək.'
        )
...
def get_order(call):
    ...
    if player_game:
        if player_game.get('order'):
            order_text = f'Sizə aşağıdakı əmri verdim: {", ".join(player_game["order"])}. Bu sırada çəkirik, əks halda darıxacağıq. ~ {role_titles["don"]}'
        else:
            order_text = f'Sifariş vermədim, oyun zamanı improvizə edirik. Əsas odur ki, eyni insanları bir gecədə vuraq, əks halda darıxacağıq. ~ {role_titles["don"]}'
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text='Donun sifarişini ala bilmirsən.'
        )
    ...
    if required_request:
        ...
        for player in required_request['players']:
            if player['id'] == call.from_user.id:
                ...
                increment_value = -1
                request_action = '$pull'
                alert_message = 'Artıq oyunda deyilsən.'
        ...
        if player_object is None:
            if len(required_request['players']) >= config.PLAYERS_COUNT_LIMIT:
                bot.answer_callback_query(
                    callback_query_id=call.id,
                    show_alert=False,
                    text='Oyun maksimum oyunçu sayından ibarətdir.'
                )
                return
            ...
            increment_value = 1
            request_action = '$push'
            alert_message = 'İndi oyundasınız.'
        ...
        keyboard.add(
            InlineKeyboardButton(
                text='Oyuna qoşulun və ya oyundan çıxın',
        ...
        bot.edit_message_text('Tətbiq artıq mövcud deyil.', chat_id=call.message.chat.id, message_id=message_id)
...
def create(message, *args, **kwargs):
    ...
    if existing_request:
        bot.send_message(message.chat.id, 'Bu söhbətdə artıq bir oyun var!', reply_to_message_id=existing_request['message_id'])
        return
    existing_game = database.games.find_one({'chat': message.chat.id, 'game': 'mafia'})
    if existing_game:
        bot.send_message(message.chat.id, 'Bu söhbətdə artıq bir oyun davam edir!')
        return
...
def start_game(message, *args, **kwargs):
    ...
    if req is not None:
        ...
        bot.send_message(message.chat.id, 'Başlaya biləcəyiniz bir oyun üçün bir tətbiqiniz yoxdur.')
...
def cancel(message, *args, **kwargs):
    ...
    if req:
        answer = 'Tətbiqiniz silindi.'
    else:
        answer = 'Oyun üçün bir tətbiqiniz yoxdur.'
    bot.send_message(message.chat.id, answer)
...
def create_poll(message, game, poll_type, suggestion):
    ...
    if existing_poll:
        bot.send_message(
            message.chat.id,
            'Bu söhbətdə onsuz da bir səs var!',
    ...
    answer = f'{poll["creator"]} təklif edir {suggestion}.'
...
def poll_vote(call):
    ...
    if poll_condition:
        ...
        if poll['type'] == 'skip':
            ...
            stop_game(player_game, reason='Oyunçular oyunu bitirmək üçün səs verdilər.')
            return
...
def callback_inline(call):
    ...
    if player_game and call.from_user.id not in player_game['played']:
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text=f'Atış oyunçuya atıldı {victim + 1}'
        )
        ...
        bot.answer_callback_query(
            callback_query_id=call.id,
            show_alert=False,
            text='Çəkilişlərdə iştirak edə bilməzsiniz'
        )
